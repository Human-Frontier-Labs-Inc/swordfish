  ...
  describe('Magic Bytes Detection', () => {
    it('should detect executable disguised as PDF', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 95,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'Extension mismatch' },
        ],
      });
    ...
    it('should detect script disguised as image', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 75,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'Script disguised as image' },
        ],
      });
  ...
  describe('Macro Detection', () => {
    it('should detect macros in Office documents', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 65,
        signals: [
          { type: 'macro_enabled', severity: 'warning', detail: 'Document contains macros' },
        ],
        macroInfo: {
          hasMacros: true,
          macroCount: 3,
    ...
    it('should flag suspicious VBA keywords', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 85,
        signals: [
          { type: 'macro_enabled', severity: 'critical', detail: 'Macros with suspicious keywords' },
        ],
        macroInfo: {
          hasMacros: true,
          macroCount: 5,
  ...
  describe('Archive Analysis', () => {
    it('should detect executable inside archive', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 80,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'Archive contains executable' },
        ],
        archiveContents: {
          entries: [
            { filename: 'readme.txt', size: 1000, isDirectory: false },
            { filename: 'setup.exe', size: 50000, isDirectory: false },
          ],
          totalFiles: 2,
          totalDirectories: 0,
          containsExecutable: true,
          containsMacroDocument: false,
    ...
    it('should detect password-protected archive', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 60,
        signals: [
          { type: 'password_protected_archive', severity: 'warning', detail: 'Password-protected archive' },
        ],
        archiveContents: {
          entries: [],
          totalFiles: 0,
    ...
    it('should detect nested archives (archive bomb prevention)', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 45,
        signals: [
          { type: 'dangerous_attachment', severity: 'warning', detail: 'Deeply nested archive' },
        ],
        archiveContents: {
          entries: [
            { filename: 'level1.zip', size: 100000, isDirectory: false },
          ],
          totalFiles: 1,
          totalDirectories: 0,
          containsExecutable: false,
  ...
  describe('Double Extension Detection', () => {
    it('should detect invoice.pdf.exe pattern', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 90,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'Double extension detected' },
          { type: 'executable', severity: 'critical', detail: 'Executable file' },
        ],
      });
    ...
    it('should detect document.doc.js pattern', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 85,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'Double extension: doc.js' },
        ],
      });
  ...
  describe('RTL Override Detection', () => {
    it('should detect RTL override filename spoofing', async () => {
      ...
      vi.mocked(analyzeAttachment).mockResolvedValue({
        ...
        riskScore: 95,
        signals: [
          { type: 'dangerous_attachment', severity: 'critical', detail: 'RTL override character detected' },
        ],
      });
    ...
    it('should aggregate risk from multiple dangerous attachments', async () => {
      ...
      vi.mocked(analyzeAttachment)
        .mockResolvedValueOnce({
          ...
          declaredType: { extension: 'exe', mimeType: 'application/octet-stream' },
          typeMismatch: false,
          isExecutable: true,
          hasMacros: false,
          isEncrypted: false,
          ...
          riskLevel: 'critical',
          riskScore: 90,
          signals: [{ type: 'dangerous_attachment', severity: 'critical', detail: 'Double extension executable' }],
        })
        .mockResolvedValueOnce({
          filename: 'payment.docm',
          detectedType: { extension: 'docm', mimeType: 'application/vnd.ms-word.document.macroEnabled.12', category: 'office', confidence: 0.95 },
          declaredType: { extension: 'docm', mimeType: 'application/vnd.ms-word.document.macroEnabled.12' },
          typeMismatch: false,
          isExecutable: false,
          hasMacros: true,
          isEncrypted: false,
          ...
          riskLevel: 'high',
          riskScore: 70,
          signals: [{ type: 'macro_enabled', severity: 'warning', detail: 'Macro-enabled document' }],
        });
