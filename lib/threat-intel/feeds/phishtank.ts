 ...
 *
 * PhishTank's public API registration has been closed since 2024. Without an
 * explicit opt-in (PHISHTANK_API_KEY) the feed returns EMPTY rather than
 * falling back to stale hardcoded sample data — stale samples give false
 * confidence that threat intel is working. OpenPhish + URLhaus carry phishing
 * detection when this feed is off. Heuristic detection (checkPhishTankUrl
 * below) is independent of the feed and remains available either way.
 */
export async function fetchPhishTankFeed(): Promise<PhishTankEntry[]> {
  ...
  try {
    ...
    // Only verified, still-online entries.
    return data.filter((entry) => entry.verified === 'yes' && entry.online === 'yes');
  } catch (error) {
